...
// covert the youtube link to embeded link
    ...
    try {
      if (liked) {
        const res = await axios.patch(
          `http://localhost:8000/api/video/${videoId}/unlike`, // API calling to unlike a video
          {},
        ...
        const res = await axios.patch(
          `http://localhost:8000/api/video/${videoId}/like`, // API calling for liking video
          {},
  ...
  const handleDislike = async () => {
    ...
    try {
      if (disliked) {
        const res = await axios.patch(
          `http://localhost:8000/api/video/${videoId}/undislike`, // API calling to unDislike
          {},
        ...
        const res = await axios.patch(
          `http://localhost:8000/api/video/${videoId}/dislike`, // API calling to dilike
          {},
          { headers: { Authorization: `Bearer ${user.token}` } }
        );
        setDisliked(true);
  ...
  const handleComment = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:8000/api/comment", // Api for posting new comment
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/comment/${id}`, // API calling for deleting a comment
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
  ...
  const handleEditSave = async (id) => {
    ...
    try {
      await axios.patch(
        `http://localhost:8000/api/comment/${id}`, // API calling for updating/editing a comment
        { text: editText },
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      // Fetch updated comments from backend
