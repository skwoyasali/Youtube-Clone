...
function YourChannel() {
  ...
  useEffect(() => {
    ...
    setError(null);
    axios
      .get(`http://localhost:8000/api/channel/${user.channelId}`)
      .then((res) => {
        setChannel(res.data);
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:8000/api/video", trimmed, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      const res = await axios.get(
        `http://localhost:8000/api/channel/${user.channelId}`
      );
      setChannel(res.data);
      setVideos(res.data.videos || []);
      setShowUpload(false);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/api/video/${editVideo._id}`,
        trimmed,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      const res = await axios.get(
  ...
  const handleChannelEditSave = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/api/updateChannel/${channel._id}`,
        trimmed,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      const res = await axios.get(
    ...
    try {
      await Promise.all(
        selectedVideos.map((id) =>
          axios.delete(`http://localhost:8000/api/video/${id}`, {
            headers: { Authorization: `Bearer ${user.token}` },
          })
        )
      );
